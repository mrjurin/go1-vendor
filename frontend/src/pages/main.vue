    ...
    <div class="flex-1 flex flex-col h-full overflow-auto">
      <div class="h-full w-full flex flex-col">
        ...
        <div class="overflow-auto">
          ...
          <div
            class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4 px-5 py-3"
          >
            <router-link to="/purchase-order">
              <div
                class="border rounded-lg p-5 h-[100px] flex flex-col justify-between"
              >
                <p class="text-xs">AVERAGE MONTHLY BUSINESS</p>
                <h2 v-if="grandTotal !== null" class="font-semibold text-3xl">
            ...
            <router-link to="/request-for-quotation">
              ...
              >
                ...
                <h2 v-if="openRfqs !== null" class="font-semibold text-3xl">
                  {{ openRfqs }}
                </h2>
                <p v-else>0</p>
              </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 px-5 pb-3">
            <div class="border rounded-xl p-5 h-[300px]">
              <Line :data="lineChartData" :options="lineChartOptions" />
            </div>
            <div class="border rounded-xl p-5 h-[300px]">
              <Bar
                :data="topSoldItemsChartData"
                :options="topSoldItemsChartOptions"
              />
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 px-5 pb-3">
            <div
              class="border rounded-xl h-[300px] flex items-center justify-center"
            >
            ...
            >
              <Doughnut
                :data="materialRequestChartData"
                :options="materialRequestChartOptions"
              />
            </div>
          </div>
        </div>
...
const fetchGrandTotal = async () => {
  try {
    const response = await fetch(
      '/api/method/go1_vendor.api.get_purchase_order_grand_total'
    )
...
const fetchOpenRfqs = async () => {
  try {
    const response = await fetch(
      '/api/method/go1_vendor.api.get_pending_quotation_count'
    )
    const data = await response.json()
...
const pendingInvoiceCount = async () => {
  try {
    const response = await fetch(
      '/api/method/go1_vendor.api.get_pending_invoice'
    )
...
const fetchPendingInwards = async () => {
  try {
    const response = await fetch(
      '/api/method/go1_vendor.api.get_pending_inwards'
    )
    const data = await response.json()
...
const fetchMonthlyGrandTotal = async () => {
  try {
    const response = await fetch(
      '/api/method/go1_vendor.api.get_monthly_grand_total'
    )
    const data = await response.json()
...
const fetchTopSoldItems = async () => {
  try {
    const response = await fetch(
      '/api/method/go1_vendor.api.get_top_sold_items'
    )
    const data = await response.json()
    ...
    if (
      ...
      Array.isArray(data.message.message.items)
    ) {
...
const fetchPurchaseOrderAnalysis = async () => {
  try {
    const response = await fetch(
      '/api/method/go1_vendor.api.get_purchase_order_status_analysis'
    )
    const data = await response.json()
...
const fetchMaterialRequestStatus = async () => {
  try {
    const response = await fetch(
      '/api/method/go1_vendor.api.get_quotation_status_counts'
    )
    const data = await response.json()
        ...
        datasets: [
          {
            ...
            data: [statusData['Pending'] || 0, statusData['Completed'] || 0],
          },
        ],
      }
    } else {
