      ...
      <div class="mb-2 border-b p-1">
        <div class="flex flex-1 items-center h-12">
          <router-link to="/Issues">
            ...
            <span v-if="paramsvalue"> {{ paramsvalue.id }}</span>
          </router-link>
        </div>
      </div>
      <div class="main-content justify-items-center grid grid-cols-1 py-10 h-[550px] overflow-auto">
        <div class="grid grid-cols-1 content-start gap-3 m-2 w-11/12 bg-white border rounded-md pb-5">
          ...
          <div class="grid grid-cols-1 p-3">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-3 justify-items-stretch text-gray-800">
              ...
              <div class="flex flex-col gap-1">
                ...
                <FormControl v-if="isEditing" type="select" v-model="statuss" :options="statusOptions"
              ...
              <div v-if="!logged_users.data" class="flex flex-col gap-1">
                ...
                <FormControl type="select" v-if="isEditing" v-model="Supplier" :options="customOption"
                  class="text-sm font-semibold rounded-md p-1" />
                <span v-else class="text-sm font-semibold py-2">{{
                  Supplier
                }}</span>
              ...
              <div class="flex flex-col gap-1">
                ...
                <FormControl v-if="isEditing" type="select" v-model="priorityy" :options="priorityOption"
                  class="text-sm font-semibold rounded-md p-1" />
                <span v-else class="text-sm font-semibold py-2">{{
                  priorityy
                }}</span><br />
              ...
              <div class="flex flex-col gap-1">
                ...
                <FormControl v-if="isEditing" type="select" v-model="issuetype" size="md" variant="subtle"
                  :options="optionIssue" placeholder="Issue type" class="mb-3" />
                <span v-else class="text-sm font-semibold py-2">{{
                  issuetype
                }}</span><br />
              </div>
              ...
              <div class="flex flex-col gap-1">
                ...
                <FormControl v-if="isEditing" type="select" v-model="company" :options="companyoption"
                  class="text-sm font-semibold rounded-md p-1" />
                <span v-else class="text-sm font-semibold py-2">{{
                  company
                }}</span>
...
const optionsSupplier = async (limit = 100000) => {
  try {
    const supplierresponse = await fetch(
      `/api/resource/Supplier?fields=["supplier_name"]&limit=${limit}`
    )
    ...
    customOption.value =
      supplierdata.data.map((user) => user.supplier_name) || []
