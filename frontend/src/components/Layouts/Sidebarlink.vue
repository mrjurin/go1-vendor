<template>
  <div
    v-if="dynamicsApp_data.data && dynamicsApp_data.data.length"
  ...
  >
    <div v-for="(row, index) in dynamicsApp_data.data" :key="index">
      <Button
        ...
        :variant="'ghost'"
        theme="gray"
        class="w-full justify-start text-gray-800 bg-white !block overflow-hidden"
        :style="isActive(row.route) ? { backgroundColor: 'white' } : {}"
      >
        <!-- Collapsed View Icon -->
        <template v-if="isCollapsed">
          <FeatherIcon
            class="w-4 text-black flex items-center"
            stroke="black"
            :stroke-width="1"
            :name="row.icon"
          />
        ...
        <template v-else>
          <div class="flex items-center">
            ...
            <div
              class="flex-shrink-0 text-sm duration-300 ease-in-out ml-2 w-auto opacity-100"
              style="float: left"
            >
              {{ row.title }}
            </div>
