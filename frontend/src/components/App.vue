  ...
  <Popover placement="right-start" class="flex w-full">
    <template #target="{ togglePopover }">
      ...
      >
        ...
        <FeatherIcon name="chevron-right" class="w-4 text-gray-600" />
      </button>
    </template>
    <template #body>
      <div
      ...
      >
        <div v-for="app in apps.data" :key="app.name">
          <a
            :href="app.route"
            class="flex flex-col gap-1.5 w-14 h-18 rounded justify-center items-center py-2 px-1 hover:bg-gray-100"
          >
